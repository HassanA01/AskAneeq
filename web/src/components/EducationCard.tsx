        ...
        <div key={edu.institution} className="rounded-xl border border-default bg-surface p-5 shadow-sm">
          <div className="flex items-start gap-3">
            <div className="w-10 h-10 bg-[var(--color-background-info-surface)] rounded-lg flex items-center justify-center flex-shrink-0">
              <GraduationCap className="w-5 h-5 text-[var(--color-text-info-soft)]" />
            </div>

            <div className="flex-1">
              <h3 className="heading-xs">{edu.institution}</h3>
              <p className="text-sm text-[var(--color-text-info-soft)] font-medium">
              ...
              {edu.highlights.length > 0 && (
                <ul className="mt-3 space-y-1">
                  {edu.highlights.map((highlight, idx) => (
                    ...
                    >
                      <span className="text-[var(--color-text-info)] mr-2 mt-0.5 text-xs">
                        &#9679;
                      </span>
                      <span>{highlight}</span>
                    </li>
